...
class TestVersionFile:
    ...
    def test_version_file_contains_valid_semver(self):
        ...
        parts = version.split('.')
        assert len(parts) == 3, f"Version must be MAJOR.MINOR.PATCH format, got: {version}"
...
class TestChangelogManipulation:
    ...
    def test_prepend_changelog_entry(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.changelog') as f:
...
class TestBumpversionConfig:
    ...
    def test_bumpversion_config_no_auto_commit(self):
        """Test that bumpversion is configured to not auto-commit."""
        config_file = Path(__file__).parent.parent / ".bumpversion.cfg"
        content = config_file.read_text()

        # Should have commit = False
        assert "commit = False" in content or "commit=False" in content, \
            ".bumpversion.cfg should have 'commit = False'"
